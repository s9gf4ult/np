...
void Candle::setOpenCost(double oc) throw(ValueError) {
  if (oc <= 0) {
    throw(ValueError(static_cast<const char*>("Open cost must be grather than 0")));
  } else if (oc > HighCost || oc < LowCost) {
    throw(ValueError("Open cost must be in low/high boundaries"));
  } else {
    this->OpenCost = oc;
  }
}
...
void Candle::setCloseCost(double cc) throw(ValueError) {
  if (cc <= 0) {
    throw(ValueError("Close cost must be grather than 0"));
  } else if (cc > HighCost || oc < LowCost) {
    throw(ValueError("Close cost must be in low/high boundaries"));
  } else {
    this->CloseCost = cc;
  }
}
...
void Candle::setLowCost(double lc) throw(ValueError) {
  if (lc <= 0) {
    throw(ValueError("Low Cost must be grather than 0"));
  } else if (lc > OpenCost || lc > CloseCost || lc > HighCost) {
    throw(ValueError("Low Cost must be lowest in the candle"));
  } else {
    this->LowCost = lc;
  }
}
...
void Candle::setHighCost(double hc) throw(ValueError) {
  if (hc <= 0) {
    throw(ValueError("High Cost must be grather than 0"));
  } else if ((hc < OpenCost) || (hc < CloseCost) || (hc < LowCost)) {
    throw(ValueError("High Cost must be lowest in the candle"));
  } else {
    this->HighCost = hc
  }
}
...
void Candle::setOpenTime(const QDateTime &ot) throw(ValueError) {
  if (ot > CloseTime) {
    throw(ValueError("OpenTime is grather than CloseTime"));
  } else {
    this->OpenTime = ot;
  }
};
...
void Candle::setCloseTime(const QDateTime &ct) throw(ValueError) {
  if (ct < OpenTime) {
    throw(ValueError("CloseTime is less than OpenTime"));
  } else {
    this->CloseTime = ct;
  }
};
...
void Candle::setVolume(double volume) throw(ValueError) {
  if (volume <= 0) {
    throw(ValueError("Volume must be grather than zero"));
  } else {
    this->Volume = volume;
  }
};
